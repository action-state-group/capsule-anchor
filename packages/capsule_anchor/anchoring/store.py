"""Durable, append-only storage for the transparency log (stdlib ``sqlite3``).

Two backends behind one small interface:

* ``InMemoryLogStore`` -- the scaffold default (``db_path=None``). Keeps the
  EXACT prior in-memory behaviour so existing tests are unaffected.
* ``SqliteLogStore`` -- a durable append-only log on disk. Reopening the same
  path restores full state (log entries + countersigned roots), which is what
  makes the CT log a real audit substrate rather than a process artifact.

Only the storage of records lives here; all crypto / chain / CT semantics stay
in ``service.py`` and ``ct.py``. No ORM -- raw ``sqlite3`` per the task rules.
"""
