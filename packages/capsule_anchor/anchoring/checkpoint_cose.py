...
def parse_and_verify_checkpoint_cose(
    ...
    ``log_id`` is an enrolled submitter -- see below) and a ``sub`` key (the
    ...
    Figure 10 + SS3), suitable for ``AnchorerService.witness_checkpoint``
    unchanged (neither ``grade`` nor ``sub`` is one of the signing-body
    fields ``_checkpoint_signing_body`` hashes, so their presence never
    changes the digest/signature math).
