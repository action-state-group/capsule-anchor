...
# tags structurally), 3+ segments (excludes 2-segment TOML headers like [build-system]), and
# never immediately followed by `(`, `:`, or `[` -- which is what an inline link, a reference
# definition, or the first half of a `[text][ref]` pair look like.
BRACKET_ID = re.compile(r"\[[a-z0-9]+(?:-[a-z0-9]+){2,}\](?![(:\[])")
